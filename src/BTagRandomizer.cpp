...
MEM::BTagRandomizer::BTagRandomizer(
    int debug, int seed_strategy,
    const std::map<DistributionType::DistributionType, TH3D>& pdf,
    int assignrnd, int compress, int nmax) {
  btag_pdfs = pdf;
  init = 0;
  debug_code = debug;
  n_tags_l = 0;
  ...
  if (debug_code & DebugVerbosity::init) {
    ...
    if (compress_csv)
      cout << "\tWARNING: Compressing csv output in [0,1]" << endl;
  }
}
...
void MEM::BTagRandomizer::init_pdfs() {
  ...
  for (size_t j = 0; j < size_t(n_jets); ++j) {
    if (!jets[j]->isSet(Observable::CSV)) {
      if (!assign_rnd) {
        cout << "BTagRandomizer::init_pdfs(): jet[" << j
             << "] has no no CSV discriminant info" << endl;
        error_code = 1;
        continue;
      } else {
    ...
    double discr = jets[j]->isSet(Observable::CSV)
                       ? jets[j]->getObs(Observable::CSV)
                       : -99.;
    if (compress_csv) discr = TMath::Max(discr, 0.);
    if (compress_csv) discr = TMath::Min(discr, 0.999999);

    // if( !jets[j]->isSet(Observable::PDGID) ) {
    //  cout << "BTagRandomizer::init_pdfs(): jet[" << j << "] has no no PDG ID
    //  info" << endl;
    //  error_code = 1;
    //  continue;
    //}
    ...
    // int mcmatch  = jets[j]->isSet(Observable::MCMATCH) ?
    // std::abs(int(jets[j]->getObs(Observable::MCMATCH))) : -1;

    if (debug_code & DebugVerbosity::init_more)
      cout << "BTagRandomizer::init_pdfs(): init jet[" << j
           << "] with (Pt,Eta,csv) = (" << jets[j]->p4().Pt() << ", "
           << jets[j]->p4().Eta()  //<< ", " << pdg
           << ", " << discr << ")" << endl;

    /*
    DistributionType::DistributionType type =
    DistributionType::DistributionType::csv_l;
    if( pdg<4 || pdg==21 ){
      type = DistributionType::DistributionType::csv_l;
      ++count_l;
      if     ( pdg==3              &&
    btag_pdfs.find(DistributionType::DistributionType::csv_s) != btag_pdfs.end()
    )
        type = DistributionType::DistributionType::csv_s;
      else if( (pdg==1 || pdg==2)  &&
    btag_pdfs.find(DistributionType::DistributionType::csv_u) != btag_pdfs.end()
    )
        type = DistributionType::DistributionType::csv_u;
      else if( pdg==21             &&
    btag_pdfs.find(DistributionType::DistributionType::csv_g) != btag_pdfs.end()
    )
      ...
      ++count_c;
      if     ( mcmatch==0 &&
    btag_pdfs.find(DistributionType::DistributionType::csv_c_g) !=
    btag_pdfs.end() )
        type = DistributionType::DistributionType::csv_c_g;
      else if( mcmatch>0  &&
    btag_pdfs.find(DistributionType::DistributionType::csv_c_t) !=
    btag_pdfs.end() )
      ...
      ++count_b;
      if     ( mcmatch==0 &&
    btag_pdfs.find(DistributionType::DistributionType::csv_b_g) !=
    btag_pdfs.end() )
        type = DistributionType::DistributionType::csv_b_g;
      else if( mcmatch>0  &&
    btag_pdfs.find(DistributionType::DistributionType::csv_b_t) !=
    btag_pdfs.end() )
    ...
    */
    ...
    if (type == DistributionType::DistributionType::Unknown ||
        btag_pdfs.find(type) == btag_pdfs.end()) {
      cout << "BTagRandomizer::init_pdfs(): jet[" << j
           << "] has no valid distribution type" << endl;
      error_code = 1;
      continue;
    }
    ...
    TH1D* h1 = h3.ProjectionZ(Form("%d_pz", int(j)), TMath::Max(binX, 1),
                              TMath::Max(binX, 1), TMath::Max(binY, 1),
                              TMath::Max(binY, 1));

    if (!h1) {
      cout << "BTagRandomizer::init_pdfs(): no histogram for jet[" << j << "] "
           << endl;
      error_code = 1;
      continue;
    }
    ...
    double norm =
        h1->Integral(compress_csv ? 1 : 0,
                     compress_csv ? h1->GetNbinsX() : h1->GetNbinsX() + 1);
    double pass =
        h1->Integral(bin, compress_csv ? h1->GetNbinsX() : h1->GetNbinsX() + 1);

    if ((norm == 0. || pass == norm || pass == 0.)) {
      if (debug_code & DebugVerbosity::init) {
        if (norm == 0.)
          cout << "BTagRandomizer::init_pdfs(): histogram for "
               << static_cast<int>(type) << " has " << norm
               << " entries: use main distribution type" << endl;
        else if (pass == norm)
          cout << "BTagRandomizer::init_pdfs(): histogram for "
               << static_cast<int>(type) << " has " << pass << "/" << norm
               << " entries passing: use main distribution type" << endl;
        else
          cout << "BTagRandomizer::init_pdfs(): histogram for "
               << static_cast<int>(type) << " has " << pass << "/" << norm
               << " entries passing: use main distribution type" << endl;
      ...
      h1 = h3.ProjectionZ(Form("%d_pz", int(j)), TMath::Max(binX, 1),
                          TMath::Max(binX, 1), TMath::Max(binY, 1),
                          TMath::Max(binY, 1));
    }
    ...
    if (compress_csv) {
      ...
      if (h1->GetBinContent(h1->GetNbinsX() + 1) > 0.)
        h1->SetBinContent(h1->GetNbinsX(),
                          h1->GetBinContent(h1->GetNbinsX()) +
                              h1->GetBinContent(h1->GetNbinsX() + 1));
    }

    if (assign_rnd) {
      for (int k = 0; k < int(ran->Uniform(1, 10)); ++k)
        discr = h1->GetRandom();
      if (debug_code & DebugVerbosity::init_more)
        cout << "BTagRandomizer::init_pdfs(): assigning random output " << discr
             << endl;
    }

    bin = h1->FindBin(cut_val);
    norm = h1->Integral(compress_csv ? 1 : 0,
                        compress_csv ? h1->GetNbinsX() : h1->GetNbinsX() + 1);
    pass =
        h1->Integral(bin, compress_csv ? h1->GetNbinsX() : h1->GetNbinsX() + 1);
    pass -= (cut_val - h1->GetBinLowEdge(bin)) / h1->GetBinWidth(bin) *
    ...
    if (debug_code & DebugVerbosity::init_more)
      cout << "BTagRandomizer::init_pdfs(): adding <TH1D*, " << pass << ","
           << discr << "> for jet[" << j << "], bin(" << binX << "," << binY
           << ")" << endl;
  }
...
vector<MEM::BTagRandomizerOutput> MEM::BTagRandomizer::run_all(
    const vector<JetCategory>& cats) {
...
bool MEM::BTagRandomizer::check_ignore_jets() {
  ...
  for (size_t j = 0; j < size_t(n_jets); ++j) {
    if (jets.at(j)->isSet(Observable::IGNORE_FOR_RND) &&
        jets.at(j)->getObs(Observable::IGNORE_FOR_RND) > 0) {
  ...
  if ((count_t + residual) < n_tags_l) {
    if (debug_code & DebugVerbosity::init_more)
      cout << "BTagRandomizer::check_ignore_jets(): only " << count_t
           << " tagged + " << residual << " residual jets, while a minimum of "
           << n_tags_l << " required" << endl;
    return false;
  }
  if (n_tags_h >= 0 && count_t > n_tags_h) {
    if (debug_code & DebugVerbosity::init_more)
      cout << "BTagRandomizer::check_ignore_jets(): already " << count_t
           << " tagged jets, while a maximum of " << n_tags_h << " required"
           << endl;
    return false;
  }
...
MEM::BTagRandomizerOutput MEM::BTagRandomizer::run() {
  ...
  for (size_t j = 0; j < n; ++j) {
    ...
    if (debug_code & DebugVerbosity::event)
      printf("\tInput value  jet[%d]: %.3f %s\n", int(j), input_btag[j],
             ((jets.at(j)->isSet(Observable::IGNORE_FOR_RND) &&
               jets.at(j)->getObs(Observable::IGNORE_FOR_RND) > 0)
                  ? " ( excluded )"
                  : ""));
  }
  ...
  // It can be that given the ignored jets, the event cannot fall into the
  // category
  if (!check_ignore_jets()) {
    if (debug_code & DebugVerbosity::init_more)
      cout << "This event should not be processed further: it "
           << "cannot pass the category cuts given the input. Return p=0"
           << endl;
    return out;
  }
  ...
  for (int n_tags = n_tags_l;
       n_tags <= (n_tags_h >= 0 ? TMath::Min(n_tags_h, n_jets) : n_jets);
       ++n_tags) {
    ...
    } while (next_combination(perm_index_copy.begin(),
                              perm_index_copy.begin() + n_tags,
                              perm_index_copy.end(), std::less<int>()));
  }
  ...
  for (int n_tags = n_tags_l;
    ...
    for (std::size_t n_perm = 0; n_perm < n_perms_max.at(n_tags); ++n_perm) {
      ...
      if (debug_code & DebugVerbosity::event)
        cout << "\tFilling perm " << count_perm << endl;
  ...
  if (h_combinations.Integral() == 0.) {
    if (debug_code & DebugVerbosity::init)
      cout << "WARNING: all " << n_perm_max
           << " combinations have probability zero!" << endl;
  }

  size_t rnd_perm = h_combinations.GetBinLowEdge(h_combinations.FindBin(
      h_combinations.Integral() > 0. ? h_combinations.GetRandom()
                                     : ran->Uniform(0, n_perm_max)));
  int n_tags_rnd{0};
  int n_perm_rnd{0};
  count_perm = 0;
  for (int n_tags = n_tags_l;
  ...
  if (debug_code & DebugVerbosity::init_more)
    cout << "Random permutation: " << rnd_perm << ", n tags: " << n_tags_rnd
         << ", n_perm: " << n_perm_rnd << endl;

  if (debug_code & DebugVerbosity::init_more) h_combinations.Print("all");

  MEM::Lock lk(n);
  auto perm = get_permutation(n_perm_rnd, n_tags_rnd);
  for (int t = 0; t < n_tags_rnd; ++t) {
    if (debug_code & DebugVerbosity::init_more)
      cout << "\tEnable lock for jet index (" << perm[t] << ")" << endl;
  ...
  for (size_t j = 0; j < n; ++j) {
    if (jets.at(j)->isSet(Observable::IGNORE_FOR_RND) &&
        jets.at(j)->getObs(Observable::IGNORE_FOR_RND) > 0) {
      if (debug_code & DebugVerbosity::init_more)
        cout << "\tIgnoring jet index (" << j << ")" << endl;
  ...
  while (ntoys < n_max_toys) {
    ++ntoys;
    if (debug_code & DebugVerbosity::event) cout << "\tToy: " << ntoys << endl;

    // number of passing jets among those that have to pass ( specified in perm
    // )
    int count_pass_t{0};

    // number of passing jets among those that DO NOT have to pass ( specified
    // in perm )
    int count_pass_u{0};
    ...
    for (size_t j = 0; j < n; ++j) {
      ...
      if (lk.get_haslock(j)) {
        if (!lk.get_lock(j)) {
          rnd_btag[j] =
              pdfs.at(j)->Integral() > 0 ? pdfs.at(j)->GetRandom() : vals.at(j);
    ...
    if (debug_code & DebugVerbosity::event)
      cout << "\tPassing:  in perm...." << count_pass_t << ", out perm...."
           << count_pass_u << ". Requested...." << n_tags_rnd << endl;
    ...
    // if the number of passing jets for this toy exceeds the maximum number,
    // reset
...
void MEM::BTagRandomizer::set_condition(const int& ntags_l, const int& ntags_h,
                                        const double& v,
                                        const unsigned long& sd) {
  n_tags_l = ntags_l;
  n_tags_h = ntags_h;
  cut_val = v;
  seed = sd;
  if (debug_code & DebugVerbosity::init_more) {
    cout << "BTagRandomizer::set_condition(): add category (" << n_tags_l << ","
         << n_tags_h << ") at cut value " << cut_val << ", seed=" << seed
         << endl;
  }
}
...
bool MEM::BTagRandomizer::consider(const std::vector<int>& perm,
                                   const std::size_t& ntag) const {
