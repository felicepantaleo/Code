...
double MEM::transfer_function2(void* obj,  // either MEM::Object or TF1
                               ...
                               int& out_of_range, const double& cutoff,
                               const bool& smear, const int& debug) {
  double w = 1.0;
  ...
  switch (type) {
    ...
    case TFType::qReco:
      ...
      if (smear) w = tf->GetRandom();

#ifdef DEBUG_MODE
      if (debug & DebugVerbosity::integration)
        cout << "\t\ttransfer_function2: Evaluate W(" << x[0] << " | "
             << _obj->p4().Pt() << ", TFType::qReco) = " << w << endl;
#endif
      break;
  ...
  while (tot > (1 - alpha) / 2 && e_L > 0.) {
    ...
    for (size_t i = 0; i < 500.; ++i) {
      ...
      if (obj == nullptr) {
        ...
        tot +=
            transfer_function2(obj, gen, type, accept, cutoff, false, debug) *
            step_size;
      }
...
TF1* MEM::Object::getTransferFunction(MEM::TFType::TFType name) {
  auto tf_it = transfer_funcs.find(name);
...
vector<vector<int>> MEM::get_permutations(std::size_t nb, std::size_t nq, std::vector<std::size_t>& lost, std::size_t isqcd){
  ...
  if(!isqcd){
  ...
  else { //DS perms for QCD hypo - light quarks ignored
    if(nq==4 && nb==4){ //8j,4b
      if(nlost==0){ //4w2h2t
	vector<vector<int>> vec = {{4,5,0,6,7,1,2,3},
				   {4,5,0,6,7,1,3,2},
	...
	perms = vec;
      ...
      else cout << "8j,4b "<< nlost <<" lost not defined QCD" << endl;
    }
    else if(nq==5 && nb==3){ //8j,3b
      if(nlost==1 && lost[0]==2){ //4w2h1t - b1
	vector<vector<int>> vec = {{3,4,-2,6,7,0,1,2},
	...
	perms = vec;
      ...
      else cout << "8j,3b "<< nlost <<" lost not defined QCD" << endl;
    }
    else if(nq==3 && nb==4){ //7j,4b
      if(nlost==1 && lost[0]==1){ //3w2h2t - qbar1
	vector<vector<int>> vec = {{4,-2,0,5,6,1,2,3},
	...
	perms = vec;
      ...
      else cout << "7j,4b "<< nlost <<" lost not defined QCD" << endl;
    }
    else if(nq==4 && nb==3){ //7j,3b
      if(nlost==1 && lost[0]==2){ //4w2h1t - b1
	vector<vector<int>> vec = {{3,4,-2,5,6,0,1,2},
	...
	perms = vec;
      ...
      else cout << "7j,3b "<< nlost <<" lost not defined QCD" << endl;
    }
    else if(nq==5 && nb==4){ //9j,4b
      if(nlost==0){ //4w2h2t
	vector<vector<int>> vec = {{4,5,0,6,7,1,2,3,8},
	...
	perms = vec;
      ...
      else cout << "9j,4b "<< nlost <<" lost not defined QCD" << endl;
    }
    else cout << "perms for "<< nq+nb <<" quarks not defined QCD" << endl;
  }
